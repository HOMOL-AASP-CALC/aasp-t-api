...
module.exports = function(var1) {
    ...
    this.colunas = [
        ...
        { titulo: 'Base', tipo: 'f', casas: 2, nome: "base",  modal: "mHorasExtrasBase"},
        { titulo: 'Divisor', tipo: 'v', casas: 2, nome: "divisor"},
        { titulo: 'Salario Hora', tipo: 'f', casas: 2, nome: "salarioHora"},
        { titulo: 'Horas extras', tipo: 'f', casas: 2, nome: "horasExtras"},
    ...
    this.calc = function () {
        ...
        var HEReflex_base1 = this.getVariavel('HEReflex_base1') 
        var HEReflex_base2 = this.getVariavel('HEReflex_base2') 
        var HEReflex_base3 = this.getVariavel('HEReflex_base3') 
        var HEReflex_base4 = this.getVariavel('HEReflex_base4') 
        var HEReflex_base5 = this.getVariavel('HEReflex_base5') 
        var HEReflex_base6 = this.getVariavel('HEReflex_base6') 
        var HEReflex_base7 = this.getVariavel('HEReflex_base7') 
        var HEReflex_base8 = this.getVariavel('HEReflex_base8') 
        var HEReflex_baseVC = this.getVariavel('HEReflex_baseVC')

        for (var i in this.planilha) {
            var l  = this.planilha[i]
            
            l.base = 0
            if (HEReflex_base1) { l.base += this.getValorPlanilha('salario', i, "salario") }
            if (HEReflex_base2 && checkGratA) { l.base += this.getValorPlanilha('salario', i, "vrGratA") }
            if (HEReflex_base3 && checkGratB) { l.base += this.getValorPlanilha('salario', i, "vrGratB") }
            if (HEReflex_base4 && checkGratC) { l.base += this.getValorPlanilha('salario', i, "vrGratC") }
            if (HEReflex_base5 && insalubridade) { l.base += this.getValorPlanilha('insalubridade', i, "valor") }
            if (HEReflex_base6 && periculosidade) { l.base += this.getValorPlanilha('periculosidade', i, "valor") }
            if (HEReflex_base7 && equiparacaoSalarial) { l.base += this.getValorPlanilha('equiparacaoSalarial', i, "diferenca") }
            if (HEReflex_base8 && diferencasSalariais) { l.base += this.getValorPlanilha('diferencasSalariais', i, "diferenca") }

            for (var avc in HEReflex_baseVC) {
                // console.log('l: 105 ------- ', HEReflex_baseVC[ avc ] ,this.getVariavel( avc ) )
                if (HEReflex_baseVC[ avc ] && this.getVariavel( avc ) ) {
            ...
            l.horasExtras = this.getValorPlanilha('conversaoHEHA', i, "he")  
