...
module.exports = function(var1) {
    ...
    this.info = {
        ...
        titulo: 'Conversão de horas de sobreaviso em horas normais', 
        prefixo: 'horasSobreavisoConversao', 
    ...
    this.calc = function () {
        ...
        var horasSobreavisoMetodo = this.getVariavel('horasSobreavisoMetodo')

        this.colunas = [] 
        this.colunas.push({titulo: 'Data', tipo: 'data',  nome: "dia"})

        this.colunas.push( { titulo: 'Horas de sobreaaviso', tipo: 'h', casas: 2, nome: "horas",  modal: "mModoDigitacaoHora"} )
        this.colunas.push( { titulo: 'Percentual', tipo: 'v', casas: 2, nome: "percentual",  modal: "mMetodologiaSobreaviso"} )
        ...
        for (var i in this.planilha) {
            ...
            if (typeof l.percentual === 'undefined') l.percentual = 33.33 

            if (l.horas.length==6) {
                h = calcUtil.hora2intHora( l.horas )
                m = calcUtil.hora2intMinuto( l.horas )
                if (modo == 's') { m = (m/60)*100 } 
                if (horasSobreavisoMetodo == 'horaPercentual') {
