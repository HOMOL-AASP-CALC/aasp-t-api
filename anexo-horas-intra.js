module.exports = function(var1) {
    ...
    this.colunas = [
        ...
        { titulo: 'Base', tipo: 'f', casas: 2, nome: "base",  modal: "mHorasIntraBase"},
        { titulo: 'Divisor', tipo: 'v', casas: 2, nome: "divisor"},
        { titulo: 'Salario Hora', tipo: 'f', casas: 2, nome: "salarioHora"},
        { titulo: 'Horas intrajornada', tipo: 'f', casas: 2, nome: "horasNormais"},
    ...
    this.calc = function () {
        ...
        var HIntra_base1 = this.getVariavel('HIntra_base1') 
        var HIntra_base2 = this.getVariavel('HIntra_base2') 
        var HIntra_base3 = this.getVariavel('HIntra_base3') 
        var HIntra_base4 = this.getVariavel('HIntra_base4') 
        var HIntra_base5 = this.getVariavel('HIntra_base5') 
        var HIntra_base6 = this.getVariavel('HIntra_base6') 
        var HIntra_base7 = this.getVariavel('HIntra_base7')
        var HIntra_base8 = this.getVariavel('HIntra_base8')
        var HIntra_baseVC = this.getVariavel('HIntra_baseVC')

        var baseVC_existe = false
        for (var avc in HIntra_baseVC) {
            if (HIntra_baseVC[ avc ] && this.getVariavel( avc ) ) {
                baseVC_existe = true
            }
        }

        if (typeof HIntra_base1 === 'undefined') { HIntra_base1 = false; }
        if (typeof HIntra_base2 === 'undefined') { HIntra_base2 = false; }
        if (typeof HIntra_base3 === 'undefined') { HIntra_base3 = false; }
        if (typeof HIntra_base4 === 'undefined') { HIntra_base4 = false; }
        if (typeof HIntra_base5 === 'undefined') { HIntra_base5 = false; }
        if (typeof HIntra_base6 === 'undefined') { HIntra_base6 = false; }
        if (typeof HIntra_base7 === 'undefined') { HIntra_base7 = false; }
        if (typeof HIntra_base8 === 'undefined') { HIntra_base8 = false; }

        if (!HIntra_base1 && !HIntra_base2 && !HIntra_base3 && !HIntra_base4 && !HIntra_base5 && !HIntra_base6 && !HIntra_base7  && !HIntra_base8 && !baseVC_existe) {
            HIntra_base1 = true 
        }
        ...
        for (var i in this.planilha) {
            ...
            l.base = 0
            if (HIntra_base1) { l.base += this.getValorPlanilha('salario', i, "salario") }
            if (HIntra_base2 && checkGratA) { l.base += this.getValorPlanilha('salario', i, "vrGratA") }
            if (HIntra_base3 && checkGratB) { l.base += this.getValorPlanilha('salario', i, "vrGratB") }
            if (HIntra_base4 && checkGratC) { l.base += this.getValorPlanilha('salario', i, "vrGratC") }
            if (HIntra_base5 && insalubridade) { l.base += this.getValorPlanilha('insalubridade', i, "valor") }
            if (HIntra_base6 && periculosidade) { l.base += this.getValorPlanilha('periculosidade', i, "valor") }
            if (HIntra_base7 && equiparacaoSalarial) { l.base += this.getValorPlanilha('equiparacaoSalarial', i, "diferenca") }
            if (HIntra_base8 && diferencasSalariais) { l.base += this.getValorPlanilha('diferencasSalariais', i, "diferenca") }
            ...
            l.horasNormais = this.getValorPlanilha('horasIntraConversao', i, "horasNormais")  
